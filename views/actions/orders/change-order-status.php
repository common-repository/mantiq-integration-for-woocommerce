<div class="row gy-3">
    ...
    <div class="col-12">
        ...
        <?php foreach (wc_get_order_statuses() as $orderStatusId => $orderStatusLabel): ?>
            <div class="form-check">
                <input class="form-check-input" type="radio" name="order-status"
                       id="order-status-<?php echo esc_attr($orderStatusId); ?>" v-model="arguments.status"
                       value="<?php echo esc_attr($orderStatusId); ?>">
                <label class="form-check-label" for="order-status-<?php echo esc_attr($orderStatusId); ?>">
                    <?php echo esc_html($orderStatusLabel); ?>
                </label>
            </div>
        <?php endforeach; ?>
    </div>
