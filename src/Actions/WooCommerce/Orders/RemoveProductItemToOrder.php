...
class RemoveProductItemToOrder extends Action
{
    public function getName()
    {
        return __('Remove product item to order', 'mantiq');
    }

    public function getDescription()
    {
        return __('Remove a product item to an order', 'mantiq');
    }
    ...
    {
        return WooCommerceIntegrationWithMantiq::getPath('views/actions/orders/remove-product-item-to-order.php');
    }
    ...
    public function invoke(ActionInvocationContext $invocation)
    {
        ...
        foreach ($userArguments['items'] as $item) {
            ...
            if (isset($orderProducts[$item['id']])) {
                $order->remove_item($orderProducts[$item['id']]->get_id());
            }
        }
