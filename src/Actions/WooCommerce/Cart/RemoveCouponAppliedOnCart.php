...
class RemoveCouponAppliedOnCart extends Action
{
    public function getName()
    {
        return __('Remove applied coupon on cart', 'mantiq');
    }

    public function getDescription()
    {
        return __('Remove an applied coupon on the current cart.', 'mantiq');
    }
    ...
    {
        return WooCommerceIntegrationWithMantiq::getPath('views/actions/cart/remove-applied-coupon-on-cart.php');
    }
    ...
    public function invoke(ActionInvocationContext $invocation)
    {
        ...
        if (WC()->cart->has_discount($userArguments['coupon'])) {
            $success = WC()->cart->remove_coupon($userArguments['coupon']);
